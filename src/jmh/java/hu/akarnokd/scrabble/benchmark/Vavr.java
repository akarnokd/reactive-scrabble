...
/**
 * Shakespeare plays Scrabble with Vavr.
 * @author José
 */
public class Vavr extends ShakespearePlaysScrabble {

    static Stream<Integer> chars(String word) {
//      return Flowable.range(0, word.length()).map(i -> (int)word.charAt(i));
      return Stream.range(0, word.length()).map(index -> (int)word.charAt(index));
  }
    ...
    public List<Entry<Integer, List<String>>> measureThroughput() {
        ...
        Function<String, Long> nBlanks =
                word -> Stream.ofAll(histoOfLetters.apply(word)
                            .entrySet())
                            .map(blank)
                            .sum().longValue();
        ...
        Function<String, Stream<Integer>> first3 = word -> chars(word).take(3);
        Function<String, Stream<Integer>> last3 = word -> chars(word).drop(Integer.max(0, word.length() - 4));

        // Stream to be maxed
        Function<String, Stream<Integer>> toBeMaxed =
            word -> first3.apply(word).appendAll(last3.apply(word));
        ...
        Function<String, Integer> score3 =
            word ->
                 2 * (score2.apply(word) + bonusForDoubleLetter.apply(word))
                 + (word.length() == 7 ? 50 : 0);

        Function<Function<String, Integer>, Map<Integer, List<String>>> buildHistoOnScore =
                score -> Stream.ofAll(shakespeareWords)
        ...
        List<Entry<Integer, List<String>>> finalList =
                Stream.ofAll(buildHistoOnScore.apply(score3).entrySet())
                    .take(3)
